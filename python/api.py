        ...
        for ievt, evt in enumerate(pixelevents):
            ...
            for ipx, px in enumerate(evt.pixels):
                ...
                if px.value > 0:
                    ...
                    if Vcal_conversion:
                        ph_cal = self.dut.roc(px.roc).ADC_to_Vcal(px.column, px.row, px.value, self.dut.roc(px.roc).ph_par0, self.dut.roc(px.roc).ph_par1, self.dut.roc(px.roc).ph_par2, self.dut.roc(px.roc).ph_par3)
                    else:
                        ph_cal = 0
